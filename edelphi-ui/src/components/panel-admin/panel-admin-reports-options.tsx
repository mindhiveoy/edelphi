...
interface Props {
  ...
  onCommentCategoriesChanged: (
    selectedCommentCategories: number[] | "ALL"
  ) => void;
...
class PanelAdminReportsOptions extends React.Component<Props, State> {
  ...
  private renderFilters = () => {
    const queryPageFilterOptions: DropdownItemProps[] = this.state.queryPages.map(
      (queryPage): DropdownItemProps => {
        return {
          key: queryPage.id,
    ...
    return (
      <div className="block">
        ...
        <div>
          <Select
            value={this.props.queryPageId}
            options={queryPageFilterOptions}
    ...
    return this.state.panelInterestClasses.map(interestClass => {
      return (
        <Table.Row>
          ...
          {this.state.panelExpertiseClasses.map(expertiseClass => {
            return this.renderExpertiseMatrixCell(
              interestClass,
              expertiseClass
            );
          })}
   ...
   */
  private renderExpertiseMatrixCell = (
    interestClass: PanelInterestClass,
    expertiseClass: PanelExpertiseClass
  ) => {
    const expertiseGroupId = this.getExpertiseGroupId(
      interestClass.id,
      expertiseClass.id
    );
    const checked =
      this.props.expertiseGroupIds === "ALL" ||
      this.props.expertiseGroupIds.indexOf(expertiseGroupId) !== -1;

    return (
      <Table.Cell
        onClick={() => this.onExpertiseGroupClick(expertiseGroupId)}
        textAlign="center"
      >
   ...
   */
  private getExpertiseGroupId = (
    interestClassId: number | undefined,
    expertiseClassId: number | undefined
  ): number => {
    const group = this.state.panelExpertiseGroups.find(panelExpertiseGroup => {
      return (
        panelExpertiseGroup.expertiseClassId === expertiseClassId &&
        panelExpertiseGroup.interestClassId === interestClassId
      );
    });

    if (!group || !group.id) {
      throw new Error(
        "Could not find expertise group for interest group / expertise group"
      );
    }
  ...
  private onExpertiseGroupClick = (expertiseGroupId: number) => {
    if (this.props.expertiseGroupIds === "ALL") {
      ...
      const result =
        this.props.expertiseGroupIds.indexOf(expertiseGroupId) !== -1
          ? _.without(this.props.expertiseGroupIds, expertiseGroupId)
          : [expertiseGroupId].concat(this.props.expertiseGroupIds);
    ...
    return (
      <div>
        ...
        {Object.keys(this.state.commentCategoryMap).map(categoryName => {
          const categoryIds = this.state.commentCategoryMap[categoryName].map(
            commentCategory => commentCategory.id!
          );
          const selectedCategoryIds = this.getCommentCategoryIds();
          const checked = !!categoryIds.find(
            categoryId => selectedCategoryIds.indexOf(categoryId) !== -1
          );
            ...
            <div>
              ...
              <Checkbox
                ...
                onChange={() => {
                  ...
                  if (checked) {
                    commentCategoryIds = selectedCategoryIds.filter(
                      selectedCategoryId => {
                        return categoryIds.indexOf(selectedCategoryId) === -1;
                      }
                    );
                  } else {
                    commentCategoryIds = _.union(
                      selectedCategoryIds,
                      categoryIds
                    );
                  }
  ...
  private renderExports = () => {
    return (
      <div className="block">
        ...
        <div>
          <a href="#" onClick={this.onExportReportContentsPdfClick}>
            {" "}
            {strings.panelAdmin.reports.exportReportPDF}{" "}
          </a>
        </div>
        <div>
          <a href="#" onClick={this.onExportReportContentsGoogleDocumentClick}>
            {" "}
            {strings.panelAdmin.reports.exportReportGoogleDocument}
          </a>
        </div>
        <h3> {strings.panelAdmin.reports.exportCharts} </h3>
        <div>
          <a href="#" onClick={this.onExportReportImagesPngClick}>
            {" "}
            {strings.panelAdmin.reports.exportChartsPNG}
          </a>
        </div>
        <h3> {strings.panelAdmin.reports.exportData} </h3>
        <div>
          <a href="#" onClick={this.onExportReportSpreadsheetCsvClick}>
            {" "}
            {strings.panelAdmin.reports.exportDataCSV}
          </a>
        </div>
        <div>
          <a href="#" onClick={this.onExportReportSpreadsheetGoogleSheetsClick}>
            {" "}
            {strings.panelAdmin.reports.exportDataGoogleSpreadsheet}
          </a>
        </div>
      </div>
    );
   ...
   */
  private onExportReportContentsPdfClick = (
   ...
   * Event handler for export as PDF click
   */
  private onExportReportContentsGoogleDocumentClick = (
    event: React.MouseEvent<HTMLAnchorElement, MouseEvent>
  ) => {
    event.preventDefault();
   ...
   */
  private onExportReportSpreadsheetCsvClick = (
    event: React.MouseEvent<HTMLAnchorElement, MouseEvent>
  ) => {
    event.preventDefault();
   ...
   */
  private onExportReportSpreadsheetGoogleSheetsClick = (
    event: React.MouseEvent<HTMLAnchorElement, MouseEvent>
  ) => {
    event.preventDefault();
   ...
   */
  private onExportReportImagesPngClick = (
    event: React.MouseEvent<HTMLAnchorElement, MouseEvent>
  ) => {
    event.preventDefault();
