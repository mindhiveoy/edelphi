...
class QueryCommentContainer extends React.Component<Props, State> {
  ...
  public async componentDidUpdate(prevProps: Props, prevState: State) {
    ...
    if (
      ...
      this.state.comments &&
      !_.isEqual(this.state.comments, prevState.comments)
    ) {
  ...
  private async onQueryQuestionCommentNotification(
    ...
    if (
      message.pageId !== this.props.pageId ||
      message.panelId !== this.props.panelId ||
      message.queryId !== this.props.queryId
    ) {
      return;
    }
    ...
    if (message.type === "CREATED" && parentCommentId === this.props.parentId) {
      const comment = await this.getQueryQuestionCommentsService(
        this.props.accessToken
      ).findQueryQuestionComment({
      ...
      for (let i = 0; i < this.state.comments.length; i++) {
        ...
        if (message.commentId === comment.id) {
          if (message.type === "UPDATED") {
            comments.push(
              await this.getQueryQuestionCommentsService(
                this.props.accessToken
              ).findQueryQuestionComment({
                panelId: this.props.panelId,
                commentId: message.commentId
          ...
          if (
            !(message.type === "DELETED" && message.commentId === comment.id)
          ) {
   ...
   */
  private compareComments = (
    a: QueryQuestionComment,
    b: QueryQuestionComment
  ): number => {
    if (
      a.parentId ||
      b.parentId ||
  ...
  private async loadChildComments() {
    if (!this.state.comments && this.props.accessToken) {
      ...
      const comments = await this.getQueryQuestionCommentsService(
        this.props.accessToken
