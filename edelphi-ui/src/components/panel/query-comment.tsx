 ...
 */
export class QueryCommentClass extends React.Component<Props, State> {
  ...
  public componentWillUnmount() {
    mqttConnection.unsubscribe(
      "queryquestioncomments",
      this.onQueryQuestionCommentNotification.bind(this)
    );
  }
    ...
    return (
      <div key={this.props.comment.id} className="queryComment">
        ...
        <div
          className={
            this.state.folded
              ? "queryCommentShowHideButton hideIcon"
              : "queryCommentShowHideButton showIcon"
          }
        ...
        <div className="queryCommentHeader" style={{ clear: "both" }}>
          <div className="queryCommentDate">
            {strings.formatString(
              strings.panel.query.comments.commentDate,
    ...
    return (
      <Confirm
        ...
        content={strings.formatString(
          strings.panel.query.comments.confirmRemoveText,
          message
        )}
  ...
  private renderContents() {
    if (this.state.commentEditorOpen) {
      return (
        <div className="editCommentEditor">
          <textarea
            ref={textarea => {
              this.commentEditor = textarea;
            }}
            onChange={event => {
              this.setState({ commentEditorContents: event.target.value });
            }}
            value={this.state.commentEditorContents}
          ></textarea>
          <input
            type="button"
            className="formButton"
            disabled={
              !this.state.commentEditorContents ||
              !this.state.commentEditorContents.trim()
            }
            onClick={(event: React.MouseEvent<HTMLElement>) =>
              this.onEditCommentSaveClick(event)
            }
    ...
    return (
      <div className="queryCommentText">{this.renderCommentContents()}</div>
    );
  }
  ...
  private renderModified() {
    ...
    return (
      <div className="queryCommentModified">
        {strings.formatString(
          strings.panel.query.comments.commentModified,
          this.formatDateTime(this.props.comment.lastModified)
        )}{" "}
      </div>
    );
  }
    ...
    return (
      <div className="newCommentEditor">
        ...
        <input
          ...
          className="formButton"
          onClick={(event: React.MouseEvent<HTMLElement>) =>
            this.onNewCommentSaveClick(event)
          }
  ...
  private renderShowHideComment() {
    ...
    return this.props.comment.hidden ? (
      <div className="queryCommentShowComment">
        <a
          style={this.state.updating ? styles.disabledLink : {}}
          href="#"
          onClick={(event: React.MouseEvent<HTMLElement>) =>
          ...
          className="queryCommentShowCommentLink"
        >
          {strings.panel.query.comments.show}
        </a>
      </div>
    ) : (
      <div className="queryCommentHideComment">
        <a
          style={this.state.updating ? styles.disabledLink : {}}
          href="#"
          onClick={(event: React.MouseEvent<HTMLElement>) =>
          ...
          className="queryCommentHideCommentLink"
        >
          {strings.panel.query.comments.hide}
        </a>
      </div>
    );
  }
   ...
   * @returns query question comments API
   */
  private getQueryQuestionCommentsService(
    accessToken: string
  ): QueryQuestionCommentsApi {
   ...
   */
  private onQueryQuestionCommentNotification(
    message: QueryQuestionCommentNotification
  ) {
    switch (message.type) {
      case "UPDATED":
