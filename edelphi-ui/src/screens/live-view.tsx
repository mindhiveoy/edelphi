...
import {
  StoreState,
  AccessToken,
  QueryQuestionCommentNotification,
...
import {
  ...
  QueryQuestionCommentsApi,
  QueriesApi,
  UsersApi,
  PanelsApi,
  QueryPagesApi,
  QueryQuestionAnswersApi,
  QueryQuestionCommentCategoriesApi
...
class LiveView extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.queryQuestionCommentsListener = this.onQueryQuestionCommentNotification.bind(
      this
    );
  ...
  public async componentDidUpdate(prevProps: Props, prevState: State) {
    if (
      ...
      !_.isEqual(this.state.categories, prevState.categories)
    ) {
      this.setState({
        categoryId: this.state.categories.length
      ...
      return (
        <Container>
          <p className="instructions">
            {strings.panelAdmin.liveView.selectQueryAndPage}
          </p>
        </Container>
      );
    }
  ...
  private renderChart() {
    ...
    const page = this.state.pages.find(p => {
      return p.id === this.state.pageId;
    });
  ...
  private renderCommentsView = () => {
    ...
    return (
      <Container
        ...
        style={{ marginTop: this.isFullscreen() ? "30px" : "0px" }}
      >
  ...
  private renderAnswersFullscreenButton = () => {
    ...
    return (
      <Button
        icon
        className="answers-fullscreen-button"
        onClick={this.onAnswersFullscreenButtonClick}
      >
  ...
  private renderCommentsFullscreenButton = () => {
    ...
    return (
      <Button
        icon
        className="comments-fullscreen-button"
        onClick={this.onCommentsFullscreenButtonClick}
      >
          ...
          <Container>
            <Form className="controls">
              <Form.Group widths="equal">
                <Form.Select
                  fluid
                  label={strings.panelAdmin.liveView.querySelectLabel}
                ...
                <Form.Select
                  fluid
                  disabled={!this.state.queryId}
                  label={strings.panelAdmin.liveView.pageSelectLabel}
                ...
                <Form.Select
                  fluid
                  disabled={!this.state.pageId}
                  label={strings.panelAdmin.liveView.commentCategorySelectLabel}
   ...
   */
  private renderCommentCell(
    commentAnswers: CommentAndAnswer[],
    x: number,
    y: number
  ) {
    ...
    const cellCommentAnswers = commentAnswers.filter(commentAnswer => {
      ...
      return (
        answer.data.x >= bounds.x1 &&
        answer.data.x <= bounds.x2 &&
        answer.data.y >= bounds.y1 &&
        answer.data.y <= bounds.y2
      );
    });
  ...
  private renderExpandCommentCellButton = (x: number, y: number) => {
    return (
      <Button
        ...
        onClick={() => this.onExpandCommentCellButtonClick(x, y)}
      >
        <Icon
          name={this.state.commentCellExpanded !== null ? "compress" : "expand"}
        />
      </Button>
    );
  };
  ...
  private renderComments(
    cellCommentAnswers: CommentAndAnswer[],
    x: number,
    y: number
  ) {
      ...
      return (
        <div key={`comment-${x}-${y}-${comment.id}`} className="comment">
          ...
          <div className="comment-contents">
            {this.renderCommentContents(comment)}
          </div>
    ...
    return (
      <div>
        <div
          ...
          key={`replies-${x}-${y}-${comment.id}`}
          onClick={onClick}
        >
          {repliesOpen ? (
            <Icon name="minus" size="small" color="green" />
          ) : (
            <Icon name="plus" size="small" color="green" />
          )}
          {strings.formatString(
            strings.panelAdmin.liveView.replyCount,
   ...
   * @param y cell y index
   */
  private renderReplyComments = (
    parent: QueryQuestionComment,
    x: number,
    y: number
  ) => {
      ...
      return (
        <div key={`${parent.id}-reply-${x}-${y}-${comment.id}`}>
          ...
          <div className="reply-children">
            {this.renderReplyComments(comment, x, y)}
          </div>
        </div>
      );
    });
  ...
  private async loadData() {
    ...
    if (
      ...
      await this.setStateAsync({
        categories: await this.getQueryQuestionCommentCategoriesService(
          this.props.accessToken.token
        ).listQueryQuestionCommentCategories({
   ...
   * @param comment comment
   */
  private getCommentAnswer(
    comment: QueryQuestionComment
  ): QueryQuestionAnswer | null {
    return (
      this.state.answers.find(answer => {
        return (
          answer.queryReplyId === comment.queryReplyId &&
          answer.queryPageId === comment.queryPageId
        );
      }) || null
    );
  }
   ...
   * @returns query question comments API
   */
  private getQueryQuestionCommentCategoriesService(
    accessToken: string
  ): QueryQuestionCommentCategoriesApi {
  ...
  private updateComment(comment: QueryQuestionComment) {
    ...
    if (
      ...
      this.state.pageId !== comment.queryPageId
    ) {
      return;
    }
  ...
  private getParentComment = (comment: QueryQuestionComment) => {
    ...
    return (
      this.state.comments.find(parentComment => {
   ...
   * @param state state
   */
  private setStateAsync<K extends keyof State>(
    state: Pick<State, K>
  ): Promise<void> {
   ...
   */
  private async onQueryQuestionCommentNotification(
    notification: QueryQuestionCommentNotification
  ) {
    if (!this.state.panel || !this.state.panel.id) {
      return;
    }
   ...
   * @param data event data
   */
  private onCategoryChange = async (
    event: React.SyntheticEvent<HTMLElement, Event>,
    data: DropdownProps
  ) => {
    await this.setStateAsync({
  ...
  private onExpandCommentCellButtonClick = (x: number, y: number) => {
    this.setState({
      commentCellExpanded:
        this.state.commentCellExpanded !== null ? null : y * 2 + x
    });
  };
